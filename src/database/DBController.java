...
public class DBController {
	...
	public void loadAppointments() 
    {
        ...
        try{
	        ...
	        rs = pst.executeQuery();
	        while (rs.next())
	        	appointments.add(new Appointment(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5), rs.getInt(6), rs.getInt(7), rs.getInt(8)));
	        				       //(FORMAT: appointmentID, patient name,doctor name,day,month,year,starthour,startminute)
        } catch (SQLException ex){
          Logger.getLogger(DBController.class.getName()).log(Level.SEVERE, null, ex);
        } 
	 ...
	 */
	 public void createAppointment(String patient, String doctor, int day, int month, int year, int starthour, int startmin) {
		 
		Random rand = new Random();
		int id = rand.nextInt(500) + 1;
		Appointment newApp = new Appointment(id, patient, doctor, day, month, year, starthour, startmin);
	  ...
	  */
	 public void deleteAppointmentForC(String patient, int day, int month, int year, int starthour, int startmin) {
	        
		 for (int i=0; i<appointments.size(); i++) {
			Appointment a = appointments.get(i);
		 	if (patient  == a.getPatient() && day == a.getDay() && month == a.getMonth() && year == a.getYear()
		 			&& starthour == a.getStartHour() && startmin == a.getStartMin()) {
		 		System.out.println("REACHED");
		 		appointments.remove(a);
		 	}
		 }
		String sql = "DELETE FROM clinic_tool.appointments WHERE Patient = ? AND Day = ? AND Month = ? AND "
				+ "Year = ? AND StartHour = ? AND StartMinute =?";
	    try {
		     pst = con.prepareStatement(sql); 
	  ...
	  */
	 public void deleteAppointmentForD(String doctor, int day, int month, int year, int starthour, int startmin) {
	 ...
	 */
	 public ObservableList<Appointment> filterbyDoctor(String doctorName){
		 ...
		 try {
	    	  pst = con.prepareStatement("SELECT AppointmentID, Patient, Doctor, Day, Month, Year, StartHour, StartMinute "
	    	  		+ "FROM clinic_tool.appointments WHERE Doctor = '" + doctorName + "'");
	    	  rs = pst.executeQuery();
	    	  while (rs.next()){
	    		  bydoctor.add(new Appointment(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5), rs.getInt(6), rs.getInt(7), rs.getInt(8)));
	    	  }
	    	  return bydoctor;
	     } catch (SQLException ex) {
	          Logger.getLogger(DBController.class.getName()).log(Level.SEVERE, null, ex);
	 ...
	 */
	 public ObservableList<Appointment> filterbyMonth(int month, int year) 
	 {
		 ...
		 try {
	    	  ...
	    	  rs = pst.executeQuery();
	    	  while (rs.next()){
	    		  byMonth.add(new Appointment(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5), rs.getInt(6), rs.getInt(7), rs.getInt(8)));
	    	  }
	    	  return byMonth;
	     } catch (SQLException ex) {
	          Logger.getLogger(DBController.class.getName()).log(Level.SEVERE, null, ex);
	 ...
	 */
	 public ObservableList<Appointment> filterbyDay(int day, int month, int year) 
	 {
		 ...
		 try {
	    	  ...
	    	  rs = pst.executeQuery();
	    	  while (rs.next()){
	    		  byDay.add(new Appointment(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5), rs.getInt(6), rs.getInt(7), rs.getInt(8)));
		      }
	    	  return byDay;
	     } catch (SQLException ex) {
	          Logger.getLogger(DBController.class.getName()).log(Level.SEVERE, null, ex);
